...
**Three pass, two fail — one a driver charset limit, one a driver's own Unicode handling — and two have no Windows driver today.** Windows' driver
...
| ingres | **FAIL** | Actian Client 1.5.0 (`actian-client-1.5.0-20-win-x86_64.zip`, esd.actian.com behind a login, Authenticode Actian Corporation; `setup.exe /r <response file>` — a bare msi installs only a DBA-tools stub); `CAIIOD35.DLL` 3.50.1211.0279, registered `Ingres`; server `iidbdb/ingres` via the compose service + `tests/compat/fixtures/setup_ingres.sh`, `INGRES 10.00.0000`. The client negotiates the server's `ISO88591` (`UTF8` refused, `E_GC2462`) and stores `?` for U+1F680 on every route (pyodbc identical), so the entry fails the astral check; on this tree it fails earlier with `08S01 (13172737) The connection to the server has been aborted` at the two-row array INSERT, because the `iiodbcdriver` key misses `caiiod35` and the Windows tail keeps the wide path — with the key extended and exempted from the tail: `FAIL AssertionError: héllo ?`. Vnode needs `encryption_mode off` (`E_GC1004_SEC_MECH_UNKNOWN`), `character_set ISO88591`, and `II_TIMEZONE_NAME=GMT` (the installer's `America-New_York` has no mapping in its `config.dat`) |
