...
Three independent re-runs of the corrected harness on a quieter host (load average
2.4-3.0, no other agent working, the same 8 partitions and 1 M rows) gave **1.410x,
1.521x and 1.372x** -- mean 1.43x, every checksum matching. The 1.21x above was taken
at load 7-9. So the honest range for the 1 M fetch is roughly **1.2x-1.5x depending on
what else the machine is doing**, and the number to quote is the low end, not the mean
