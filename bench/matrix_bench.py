...
TABLE = "adbc_bench" + m.SUFFIX  # ADBC_MATRIX_SUFFIX isolates concurrent runs
...
def make_table(n, ident=lambda x: x):
    # Bulk ingest quotes column names, so they must match how the server stored
    # them in CREATE TABLE (Oracle folds unquoted names to upper case).
    return pa.table({
        ident("id"): pa.array(range(n), pa.int32()),
        ident("val"): pa.array([i * 0.5 for i in range(n)], pa.float64()),
        ident("txt"): pa.array(["row-%012d" % i for i in range(n)]),
        ident("dt"): pa.array([i % 20000 for i in range(n)], pa.date32()),
    })
...
def bench(name, cfg):
    ...
    # "delegate": "never" keeps this measuring the ODBC path, not a native driver.
    conn = dbapi.connect(driver=m.DRIVER,
...
lines = ["<!-- SPDX-License-Identifier: Apache-2.0 -->", "# Per-database benchmarks", "",
         "Full fetch of the row count each line names (`--rows`, default %d) of "
         "`(int, double, varchar(20), date)`; median of %d runs after a warmup; "
...
for name, r in results.items():
    ...
    lines.append("| %s (%s) | %d | %.2f s | **%s** | **%.3f s** | %s | %s | %s |" % (
        name, r["vendor"], r["rows"], r["ingest"], "{:,.0f}".format(r["rows"] / r["ingest"]), r["adbc"],
