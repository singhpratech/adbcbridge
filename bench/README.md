...
| Windows 11 (x64, i7-8550U, 8 GB, the OS's own driver manager) | [`BENCHMARKS-windows.md`](BENCHMARKS-windows.md) — SQLite, DuckDB, SQL Server 2025, PostgreSQL 16, MySQL 8.4; [`LANGUAGE_BENCHMARKS-windows.md`](LANGUAGE_BENCHMARKS-windows.md) — five languages × five databases, 24 of 25 cells | **measured, one machine, single samples, campaign closed** (first build ever on 2026-08-24; ten defects found and fixed the same day, three of them silent text corruption from the ANSI code page; no prefetch and no ingest fan-out in the Windows build, 26% run-to-run variance on the box, so not comparable with Linux; the 41 other entries carry the reason each did not run — no container runtime on the box for 36 of them) |
