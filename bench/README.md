...
| Windows 11 (x64, i7-8550U, 8 GB, the OS's own driver manager) | [`BENCHMARKS-windows.md`](BENCHMARKS-windows.md) — SQLite, DuckDB, SQL Server 2025, PostgreSQL 16, MySQL 8.4, then CockroachDB, TimescaleDB, Citus, CrateDB, QuestDB, TiDB, Dolt, Percona, ArcadeDB, RisingWave, Materialize, YugabyteDB, openGauss, Spanner, Cloudberry, ClickHouse, YDB, Azure SQL Edge, ColumnStore, MonetDB under Docker Desktop and MariaDB 12.3 natively; [`LANGUAGE_BENCHMARKS-windows.md`](LANGUAGE_BENCHMARKS-windows.md) — five languages × five databases, 24 of 25 cells | **measured, one machine, single samples, campaign closed at 26 of 46** (first build ever on 2026-08-24; ten defects found and fixed the same day across driver, tests and harnesses, three of them silent text corruption from the ANSI code page; verified at `b5d2791`; no prefetch and no ingest fan-out in the Windows build, 26% run-to-run variance on the box, so not comparable with Linux; every other entry carries its reason or `pending`) |
