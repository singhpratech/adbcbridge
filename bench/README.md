...
| Windows 11 (x64, i7-8550U, 8 GB, the OS's own driver manager) | [`BENCHMARKS-windows.md`](BENCHMARKS-windows.md) — SQLite, DuckDB, SQL Server 2025, PostgreSQL 16, MySQL 8.4; [`LANGUAGE_BENCHMARKS-windows.md`](LANGUAGE_BENCHMARKS-windows.md) — five languages × five databases, 24 of 25 cells | **measured, one machine, single samples** (first build ever on 2026-08-24; two text-encoding bugs found and fixed the same day; no prefetch and no ingest fan-out in the Windows build, and 26% run-to-run variance on the box, so not comparable with Linux) |
