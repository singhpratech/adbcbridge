...
struct OdbcReaderOptions {
  ...
  // Server requirement: every table must have a primary key, so the CREATE TABLE bulk
  // ingest generates has to declare one.  This is the column definition appended after
  // the ingested columns to do that -- a surrogate key the server fills in itself, e.g.
  // Google Cloud Spanner's
  //   "adbc_ingest_key" bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
  // Spanner refuses any other table outright ("Primary key must be defined for table"),
  // and none of the ingested columns can serve as the key: an ingest payload may repeat
  // a value, or hold NULL, in any column.  The extra column is never written to -- the
  // generated INSERT names the ingested columns explicitly -- and reads back as whatever
  // the server generated.  NULL everywhere else: the CREATE TABLE ends after the
  // ingested columns.
  const char* ingest_key_column;
