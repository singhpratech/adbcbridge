...
| `psqlodbc` (any server) | every PostgreSQL-wire database | `SET TIME ZONE 'UTC'` at connect (`adbc.odbc.utc_session`, default on) | psqlodbc hands a `timestamptz` over as the session's wall-clock time with no offset in its `SQL_C_CHAR` form, and sends a bound timestamp as a zone-less literal the server reads in the session zone, so zoned values are only right when the session zone is UTC; on a server set to another zone every read and ingest was shifted by that zone's offset (found 2026-09-15 on a macOS cluster whose `initdb` took the host zone). A server that does not take the statement is left as it is |
