...
| Windows: per-connection narrow-text switch for ANSI-only drivers (no `W` entry point) — routes statement text and fetch through the narrow UTF-8 path, detected via `GetProcAddress(SQL_DRIVER_HLIB, "SQLExecDirectW")` — so Apache Ignite passes on Windows (prototype measured green; needs a Windows machine to land and re-measure) | next |
