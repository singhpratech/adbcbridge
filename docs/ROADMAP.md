...
| Trust artefacts: security policy with private vulnerability reporting and a stated support window (`SECURITY.md`); Dependabot on every manifest; `SHA256SUMS` on v0.1.0; every later release signed (`SHA256SUMS.asc`), attested (GitHub build provenance, `gh attestation verify`) and shipped with an SPDX SBOM by the release workflow | done |
